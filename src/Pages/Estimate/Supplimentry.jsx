...
const Supplimentry = () => {
  ...
  const handleJobSearchBtn = async(e)=>{
    ...
    if(!jobID){
      ...
      await axios.get(`http://localhost:8080/api/estimate/searchJob/${jobID}`)
      .then((res)=>{
        ...
        axios.get(`http://localhost:8080/api/supplimentryPreview/getSupplimentryCount`, {
          ...
          console.log(supplimentryCounts)
        }).catch((err)=>{
          console.log(err.message)
        })
...
const paintUpdateHandler = () => {
  ...
  const handleSupplimentry = async () => {
    ...
    if(!formData.data || formData.data.length === 0){
      ...
      await axios.post(`http://localhost:8080/api/supplimentry/createSupplimentry`, formData)
      .then((res) => {
        setErrorVisible("block");
      ...
      await axios.get("http://localhost:8080/api/supplimentryPreview/updateSupplimentryCount", {
